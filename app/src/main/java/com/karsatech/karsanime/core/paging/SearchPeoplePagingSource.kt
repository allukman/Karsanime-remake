...
class SearchPeoplePagingSource (private val peopleService: PeopleService, private val query: String): PagingSource<Int, DetailPeopleItem>() {

    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, DetailPeopleItem> =
        try {
            val position = params.key ?: INITIAL_PAGE_INDEX

            val responseData = peopleService.searchPeople(query = query,position, 10).data
