...
class ThisSeasonAnimePagingSource (private val animeService: AnimeService): PagingSource<Int, DetailAnimeItem>() {

    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, DetailAnimeItem> =
        try {
            val position = params.key ?: INITIAL_PAGE_INDEX
            val responseData = animeService.getAnimeThisSeason(position, 25).data
