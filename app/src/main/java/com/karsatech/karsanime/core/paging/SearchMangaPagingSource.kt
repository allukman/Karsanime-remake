...
class SearchMangaPagingSource(private val mangaService: MangaService, private val query: String) :
    PagingSource<Int, DetailAnimeItem>() {

    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, DetailAnimeItem> =
        try {
            val position = params.key ?: INITIAL_PAGE_INDEX

            val responseData = mangaService.searchManga(query = query, position, 10).data
